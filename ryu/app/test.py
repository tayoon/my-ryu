...
class SimpleSwitch(app_manager.RyuApp):
    ...
    def add_flow(self, datapath, in_port, dst, src, actions):
        ...
        mod = parser.OFPFlowMod(
            ...
            command=ofproto.OFPFC_ADD, idle_timeout=MAC_IDLE_TIME, actions=actions)
    ...
    def modify_flow(self, datapath, mac, port):
        ...
        actions = [datapath.ofproto_parser.OFPActionOutpu(port)]

        mod = parser.OFPFlowMod(datapath=datapath, match=match, 
                                cookie=0, idle_timeout=MAC_IDLE_TIME, command=ofproto.OFPFC_MODIFY, actions=actions)
        datapath.send_msg(mod)
